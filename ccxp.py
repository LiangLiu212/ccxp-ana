...
class ccxp:
    ...
    def _processing_ntuple(self, ifunc, run = ["run1"]):
        # processing ntuple don't drop events and
        for r, sample in self.ntuples.items():
            for s, nt in sample.items():
                ifunc(r, s, nt)

    def _apply_selection(self):
        # selection will drop events, and it must be apply for all ntuples with the same
        for r, sample in self.ntuples.items():
            for s, nt in sample.items():
                sel = selection(nt)
                sel.execute()
